    ...
    CVI_S32 s32InputSizeBytes, CVI_U32* pu32OutLen)
{
    ...
    else if (pstEncoder->stAACAttr.enAACType == AAC_TYPE_EAAC || pstEncoder->stAACAttr.enAACType == AAC_TYPE_EAACPLUS)
        u32WaterLine = AACPLUS_SAMPLES_PER_FRAME;
    else if (pstEncoder->stAACAttr.enAACType == AAC_TYPE_AACLD || pstEncoder->stAACAttr.enAACType == AAC_TYPE_AACELD)
    ...
    CVI_U16* pu16Outbuf, CVI_U32* pu32OutLen, CVI_U32* pu32Chns)
{
    ...
    if ((s32Samples != AACLC_SAMPLES_PER_FRAME) && (s32Samples != AACPLUS_SAMPLES_PER_FRAME) && (s32Samples != AACLD_SAMPLES_PER_FRAME)) {
...
static CVI_BOOL _cvi_checkname_iswav(char* infilename)
{
    ...
    if (infilename[s32InputFileLen - 4] == '.' && (infilename[s32InputFileLen - 3] == 'W' || infilename[s32InputFileLen - 3] == 'w') && (infilename[s32InputFileLen - 2] == 'A' || infilename[s32InputFileLen - 2] == 'a') && (infilename[s32InputFileLen - 1] == 'V' || infilename[s32InputFileLen - 1] == 'v')) {
...
int main(int argc, char** argv)
{
    ...
    if (bEncodeAAC == true) {
        ...
        else if (aac_config.enAACType == AAC_TYPE_EAAC || aac_config.enAACType == AAC_TYPE_EAACPLUS)
            s32framesize = AACPLUS_SAMPLES_PER_FRAME;
        else if (aac_config.enAACType == AAC_TYPE_AACLD || aac_config.enAACType == AAC_TYPE_AACELD)
