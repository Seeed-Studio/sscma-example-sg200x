    ...
    audio_frame_handler frame_out, audio_stream_handler stream_out)
{
    ...
    AUDIO_SAMPLE_RATE_E Chnsample_rate = rate;

    AIO_ATTR_S AudinAttr;

    AudinAttr.enSamplerate = rate;
    ...
    if (bVqe) {
        ...
        if (((AudinAttr.enSamplerate == AUDIO_SAMPLE_RATE_8000) || (AudinAttr.enSamplerate == AUDIO_SAMPLE_RATE_16000)) && channel == 2) {
