...
int main(int argc, char** argv) {
    ...
    MA_LOGD(TAG, "start %s %s", __DATE__, __TIME__);
    MQTT mqtt("test", "tx", "rx");
    if (mqtt.connect("192.168.42.3", 1883)) {
    ...
    while (1) {
        ...
        size_t len = 0;
        if ((len = transport->receiveUtil(buf, sizeof(buf), 0x0D)) > 0) {
            server.execute(buf, transport);
        }
    }
