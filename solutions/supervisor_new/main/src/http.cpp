...
static void registermodeldetector(HttpService& router){
    router.GET("/modeldetector", [](HttpRequest* req, HttpResponse* resp) {
        ...
        resi["code"] = 0;
        resi["msg"]  = "";
        resi["data"] = model_detector("yolo11n_cv181x_int8.cvimodel");
        resi["version"]      = PROJECT_VERSION;
        resi["uptime"]    = getUptime();
        resi["timestamp"] = getTimestamp();

        return resp->Json(resi);
    });
}
